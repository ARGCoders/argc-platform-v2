...
| **Blog follows `BLOG_DESIGN_SPECS.md`**           | The blog was redesigned to its own spec (added with the redesign): no maroon hero, paper bg with a massive ink title, stone cards with 1px borders and grayscale→colour images, bracket tags, 65ch article column. The navbar gets the `light` variant on `/blog`. |
