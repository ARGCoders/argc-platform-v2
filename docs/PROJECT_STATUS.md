...
| Blog (read-only)   | Public index + post pages, data layer, sanitised rendering, 4 components      |
...
| **Blog follows `BLOG_DESIGN_SPECS.md`**           | The blog was redesigned to its own spec (added with the redesign): no maroon hero, paper bg with a massive ink title, a wide (`90rem`) row-based index (`PostRow`) with grayscale→colour 16:9 thumbnails and bracket tags, an `80rem` single-column article page with a `46rem` body, and a maroon navbar on every page (never navy). Page navigation cross-fades via the View Transitions API. |
