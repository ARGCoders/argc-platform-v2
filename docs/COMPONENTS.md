...
| ------------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Navbar`      | `components/shared/navbar.tsx`                 | Sticky, scroll-aware, mobile hamburger with scroll lock and a profile dropdown. Links come from `content/site.json`. Reads auth from `useAuth()` — do not add another session fetch. `maroon` variant on `/events` and `/register`; `/blog` always uses the scrolled navy state.                      |
...
| `PostCard`    | `components/features/blog/post-card.tsx`       | Whole-card `Link` to `/blog/[slug]`. Stone card, 1px border; 16:9 banner (grayscale → colour on hover) via `next/image` or `BannerPlaceholder`; `Tag` brackets, `AuthorLine`, mono date + read time. Transition lift + 2px maroon bottom highlight on hover. Data is a `PostView` from `lib/blog.ts`. |
