...
function controlClasses(surface: Surface, readOnly?: boolean) {
  return cn(
    ...
    surface === "light"
      ? "border border-eng-navy/20 text-eng-navy placeholder:text-eng-navy/30 focus:border-eng-navy"
      : "border border-hero-ink/25 text-hero-ink placeholder:text-hero-ink/30 focus:border-hero-ink",
...
export const FieldArea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  function FieldArea(
    { label, error, id, required, surface = "dark", ...rest },
    ref,
  ) {
    const generated = useId();
    const fieldId = id ?? generated;
